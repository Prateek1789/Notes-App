class NoteFactory {
    ...
    createNote(id, title, colour, dateObj) {
        ...
        note.innerHTML = `<textarea name="content" id="note-${id}" class="note-content" autofocus="on"></textarea>
                          <div class="note-header">
                            <h3>${title}</h3>
                          </div>
                          <div class="note-footer">
                                <span class="note-time">${dateObj.date} • ${dateObj.time}</span>
    ...
    getColourMap() {
        return this.noteColourMap;
    }

    getColourForMode(isDarkMode, index) {
        const displayColour = isDarkMode ? [...this.noteColourMap.values()] : [...this.noteColourMap.keys()];
        return displayColour[index];
    }
} 